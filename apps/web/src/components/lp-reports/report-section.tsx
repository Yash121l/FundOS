...
function renderMarkdown(md: string): string {
  // Tables must be processed as complete blocks BEFORE the \n\n → <br> pass,
  // otherwise separator rows (|---|---|) that don't start with "| " end up as
  // literal text and the greedy row-grouping regex splits the table in two.
  let html = md.replace(/((?:^\|.+\|\n?)+)/gm, (block) => {
    const lines = block.trim().split('\n')
    const rows = lines.filter((l) => !/^\|[\s\-:|]+\|$/.test(l.trim())) // drop separator rows
    if (rows.length === 0) return ''
    const [header, ...body] = rows
    const makeRow = (line: string, tag: 'th' | 'td') => {
      const cells = line.split('|').filter(Boolean).map((c) => c.trim()).filter(Boolean)
      return `<tr>${cells.map((c) => `<${tag} class="px-3 py-1.5 text-[12px] border border-border/50">${c}</${tag}>`).join('')}</tr>`
    }
    return (
      `<table class="w-full border-collapse border border-border/50 rounded-md overflow-hidden my-3 text-left">` +
      makeRow(header!, 'th') +
      body.map((l) => makeRow(l, 'td')).join('') +
      `</table>`
    )
  })
